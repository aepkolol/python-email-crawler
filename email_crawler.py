...
email_regex = re.compile('(?!(?:.*jpg|.*png|.*gif))([A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4})', re.IGNORECASE)
url_regex = re.compile('(?!(?:.*facebook|.*twitter|.*linkedin|.*instagram))<a\s.*?href=[\'"](.*?)[\'"].*?>', re.IGNORECASE)
